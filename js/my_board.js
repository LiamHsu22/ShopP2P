...
async function show(user, n) {
    ...
    if(user == "seller"){
        ...
        id = await contract.functions.FindUserID(board[2]);
    }
    else {
        board = await contract.functions.ViewBuyerBoard(addr, n);
        id = await contract.functions.FindUserID(board[3]);
    }
    ...
    document.getElementById(n+'_msg_'+user).textContent = board[4];
    var date = new Date(parseInt(board[5])*1000);
